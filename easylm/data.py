...
class DatasetFactory(object):
    ...
    def load_dataset(cls, config, tokenizer, **kwargs):
        ...
        if config.type == 'huggingface':
            ...
            # lsp
            return JsonDataset(config, tokenizer, text_processor, **kwargs)
        else:
...
#         return token_buffer, loss_mask_buffer, *aux
    ...
    def __call__(self, example, has_aux=False):
        ...
        inputs = instruction.strip() + '\n\n' + inputs.strip() 
        target = '\n' + output.strip()
        inputs_tokens = [self.tokenizer.bos_token_id] + self.tokenizer.encode(inputs)
        loss_mask_buffer = len(inputs_tokens) * [0]
        target_tokens = self.tokenizer.encode(target) + [self.tokenizer.eos_token_id]
        loss_mask_buffer.extend(len(target_tokens) * [1])
        token_buffer = inputs_tokens + target_tokens
        
        assert len(loss_mask_buffer) == len(token_buffer), print(len(loss_mask_buffer), len(token_buffer))
        return token_buffer, loss_mask_buffer, *aux
