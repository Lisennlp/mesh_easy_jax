...
class CausalTransformerShard(hk.Module):
    ...
    def eval(self, context, target, z_loss=0., mask=0.0):  # XDC: why not merge into loss?
        ...
        x = hk.dropout(next(self.key), rate=self.config.get('embd_pdrop', 0), x=x)   # lsp: add drop
...
def param_shapes_to_shard(param_shapes, parallel, config=None):  # XD
 ...
 #           print(f'name: {name}')
            ...
            elif any(name.endswith(s) for s in ['/projection_shard/~/linear', '/fc_in', 'q_proj', 'k_proj', 'v_proj', '/fc_in3', '/fc_in1']):
...
#             elif any(name.endswith(s) for s in ['/fc_out', 'o_proj']):
#                 d[k] = P(parallel, None)   # assert shape_dtype.shape[1] == config["d_model"]
            else: assert False
    return params_shard
...
class CausalTransformer:
    def __init__(self, config):
        ...
        def train(state, ctx, tgt):
            print(f'ctxooshape: {ctx.shape} tgtshape: {tgt.shape}')
...
#             head_print(f'opt_state00: {state["opt_state"]}')  # float32
#             head_print(f'params00:{state["params"]}') # float32
#             head_print(f'grad00: {grad}') # bfloat16
#             head_print(f'optimizer: {optimizer}') # bfloat16
#             optimizer: GradientTransformation(init=<function chain.<locals>.init_fn at 0x7fa3f7d32170>, update=<function chain.<locals>.update_fn at 0x7fa3f7d32200>)
            updates, new_opt_state = optimizer.update(grad, state["opt_state"], state["params"])
#             head_print(f'updates00: {updates}')  # 32
#             head_print(f'new_opt_state00: {new_opt_state}') # 32
